...
import { commitLineasImport, parseLineasImportBuffer } from "@/lib/lineas-import";
...
/**
 * Importa líneas en lote desde un archivo Excel/CSV.
 *
  ...
  try {
    ...
    if (dryRun) {
      return NextResponse.json({
        ...
        preview: parsed.rows.slice(0, 60),
        errors: parsed.errors,
      });
    }

    const commit = await commitLineasImport(parsed.rows);

    await writeAuditEvent({
      userId: actor.userId,
      action: "catalog.import_lineas",
      detail: `Importación masiva de líneas: ${commit.created} creadas, ${commit.skippedExisting} ya existentes, ${
        commit.errors.length + parsed.errors.length
      } con error`,
    });
    ...
    console.error("Error importing lineas:", error);
    return NextResponse.json(
