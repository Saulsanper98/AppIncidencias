...
/**
 * Dato inline sin caja (icono + valor). Para filas de contexto en el hero.
 */
function MetaFact({
  icon,
  value,
  ...
  return (
    <span title={title} className="inline-flex items-center gap-1">
      {icon ? (
        <span className="shrink-0 text-[var(--color-text-3)]/75" aria-hidden>
          {icon}
        </span>
      ) : null}
      <span
...
function MetaDot() {
  return (
    <span className="select-none text-[var(--color-text-3)]/35" aria-hidden>
      ·
    </span>
  );
}
...
export default function TicketDetailPage() {
  ...
  const detailActionsMenuRef = useRef<HTMLUListElement | null>(null);
  ...
  const positionDetailActionsMenu = useCallback(() => {
    const btn = detailActionsTriggerRef.current;
    const menu = detailActionsMenuRef.current;
    if (!btn) return;
    ...
    const openUp = menuH + gap > spaceBelow && spaceAbove >= menuH + gap;
    const top = openUp
      ? Math.max(8, r.top - menuH - gap)
      : Math.min(window.innerHeight - menuH - 8, r.bottom + gap);
  ...
  useLayoutEffect(() => {
    ...
    positionDetailActionsMenu();
    const raf = window.requestAnimationFrame(() => positionDetailActionsMenu());
    window.addEventListener("scroll", positionDetailActionsMenu, true);
    window.addEventListener("resize", positionDetailActionsMenu);
    return () => {
      window.cancelAnimationFrame(raf);
      window.removeEventListener("scroll", positionDetailActionsMenu, true);
      window.removeEventListener("resize", positionDetailActionsMenu);
    };
  }, [detailActionsOpen, ticket?.status, role, positionDetailActionsMenu]);

  useEffect(() => {
    if (!detailActionsOpen) return;
    const menu = detailActionsMenuRef.current;
    if (!menu) return;
    const ro = new ResizeObserver(() => positionDetailActionsMenu());
    ro.observe(menu);
    positionDetailActionsMenu();
  ...
  const copyShortTicketId = async () => {
    ...
    const ok = await copyTextToClipboard(short);
    if (ok) {
  ...
  return (
    <div className="ticket-detail-print-root space-y-4 pb-28 [padding-bottom:max(7rem,env(safe-area-inset-bottom))] lg:pb-0 lg:[padding-bottom:unset]">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          canShowTicketEdit(
            role,
            ticket.status,
            sessionUser.isReadOnly ?? false,
            Boolean(ticket.needsCompletion),
            sessionUser.id,
            ticket,
          ) ? (
            <button
          ...
          {canRequestTicketDeletion(role) &&
          sessionUser &&
          isTicketOwnedByActor(ticket, sessionUser.id) ? (
            <button
              type="button"
              onClick={() => setRequestDeletionOpen(true)}
              className="desvios-action-chip"
              title="Solicitar eliminación a un gestor"
            >
              <Trash2 size={12} strokeWidth={1.8} aria-hidden />
      ...
      <RequestTicketDeletionDialog
        ...
        ticketLabel={`#${ticket.id.slice(-8).toUpperCase()} · ${ticket.title}`}
        onClose={() => setRequestDeletionOpen(false)}
        onSubmitted={() => {
          setRequestDeletionOpen(false);
          setNotice("Solicitud enviada. Un gestor la revisará pronto.");
        }}
      />
          ...
          <div className="tickets-hero-glow relative min-h-0 overflow-hidden rounded-xl border border-[var(--color-border)] bg-gradient-to-br from-[var(--color-surface)] via-[var(--color-surface)] to-[var(--color-accent-light)]/15 p-4 shadow-sm transition-shadow duration-200 hover:shadow-md sm:p-6">
            ...
            <div className="mb-3 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div className="min-w-0 flex-1 md:pr-2">
                ...
                <p className="mt-1 text-[13px] leading-snug text-[var(--color-text-3)]">
                ...
                <div className="mt-2.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[12.5px]">
                  ...
                  />
                  <MetaDot />
                  <MetaFact icon={<Building2 size={13} strokeWidth={1.8} />} value={ticket.operator} title="Operadora" />
                  <MetaDot />
                  <MetaFact icon={<MapPin size={13} strokeWidth={1.8} />} value={ticket.municipio} title="Municipio" />
                  {ticket.lineaLabel ? (
                    <>
                      <MetaDot />
                      <MetaFact icon={<Route size={13} strokeWidth={1.8} />} value={ticket.lineaLabel} title="Línea" />
                    </>
                  ) : null}
                  {ticket.servicioLabel ? (
                    <>
                      <MetaDot />
                      <MetaFact icon={<Hash size={13} strokeWidth={1.8} />} value={ticket.servicioLabel} title="Servicio" mono />
                    </>
                  ) : null}
                  {ticket.conductorLabel ? (
                    <>
                      <MetaDot />
                      <MetaFact icon={<UserIcon size={13} strokeWidth={1.8} />} value={ticket.conductorLabel} title="Conductor" mono />
                    </>
                  ) : null}
                </div>
                <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11.5px] text-[var(--color-text-3)]">
                ...
                <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11.5px]">
                  ...
                  >
                    {slaMinsRemaining <= 0 ? (
                      <AlertTriangle size={12} strokeWidth={1.8} className="shrink-0" aria-hidden />
                    ) : (
                      <Clock3 size={12} strokeWidth={1.8} className="shrink-0 opacity-90" aria-hidden />
                    )}
                    {slaMinsRemaining <= 0
              ...
              <div className="flex shrink-0 flex-row items-start gap-4 self-stretch border-t border-[var(--color-border)]/60 pt-3 md:border-t-0 md:pt-0">
                <div className="flex min-w-[5.5rem] flex-col gap-1">
                  ...
                  <Badge
                    className={cn(
                      "w-fit whitespace-nowrap text-xs font-semibold",
                    ...
                    <div className="flex min-w-[5.5rem] flex-col gap-1">
                      ...
                      <span className="inline-flex items-center gap-1.5">
                        ...
                        <Badge
                          variant={pr.variant}
                          className={cn(
                            "w-fit whitespace-nowrap rounded-md px-2 py-0.5 text-[10px] font-semibold",
                            pr.className,
                          )}
                        >
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-3 border-t border-[var(--color-border)]/80 bg-[var(--color-surface)]/40 px-3 py-2">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--color-accent)] px-3.5 py-1.5 text-[12px] font-semibold text-white shadow-sm transition-all hover:opacity-90 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-45 disabled:shadow-none"
                    >
                      {noteSaving ? (
                        <>
                          <span
                        ...
                        {href && !isVideo ? (
                          <div className="pointer-events-none absolute inset-0 flex items-start justify-end gap-1 bg-gradient-to-t from-black/55 to-transparent p-2 opacity-100 transition-opacity md:pointer-events-none md:opacity-0 md:group-hover:pointer-events-auto md:group-hover:opacity-100">
                            ...
                            <button
                              ...
                              title="Copiar enlace"
                              onClick={() =>
                                void copyTextToClipboard(`${window.location.origin}${href}`).then((ok) => {
                                  if (!ok) toast.error("No se pudo copiar el enlace.");
                                })
                              }
                            >
              ...
              {detailTransitions.length > 0 ? (
                <div className="relative" data-ticket-detail-actions>
                  <button
                    ...
                    type="button"
                    className={cn(
                      "flex w-full items-center justify-between gap-2 rounded-lg border px-3 py-2.5 text-sm font-semibold transition-all",
                      detailActionsOpen
                        ? "border-[var(--color-accent)]/55 bg-[var(--color-accent-light)] text-[var(--color-accent)] ring-2 ring-[var(--color-accent)]/20"
                        : "border-[var(--color-border)] bg-[var(--color-surface-2)] text-[var(--color-text-1)] hover:border-[var(--color-accent)]/40 hover:bg-[var(--color-surface-2)]",
                    )}
                  ...
                  >
                    <span className="inline-flex items-center gap-2">
                      <PlayCircle size={16} className="shrink-0 text-[var(--color-accent)]" aria-hidden />
                      Cambiar estado
                    </span>
                    <ChevronDown
                      size={14}
                      className={cn("shrink-0 text-[var(--color-text-3)] transition-transform duration-200", detailActionsOpen && "rotate-180")}
                      aria-hidden
                    />
                  </button>
                </div>
      ...
      {detailActionsOpen
        ? createPortal(
            <ul
              ...
              style={{
                position: "fixed",
                top: detailActionsMenuBox?.top ?? -9999,
                left: detailActionsMenuBox?.left ?? 0,
                width: detailActionsMenuBox?.width ?? 220,
                visibility: detailActionsMenuBox ? "visible" : "hidden",
                zIndex: 120,
              }}
              className="max-h-[min(50vh,16rem)] overflow-y-auto rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] py-1 shadow-2xl ring-1 ring-black/10"
            >
